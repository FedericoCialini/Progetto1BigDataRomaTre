...
def elaborate(spark, sectorPair):
    Tickers = historical_stock_price(spark)\
        ...
        .reduceByKey(lambda x, y: x + y)\
        .map(lambda x: (x[0], [[finalVariance(x[1][0][0], x[1][-1][0]), avgColumn(x[1], 1), sum(TakeColumn(x[1], 2)), len(x[1])]]))\
        .map(lambda x: ((getSector(sectorPair, x[0][0]), x[0][1]), x[1]))\
        .reduceByKey(lambda x, y: x + y)\
...
if __name__ == "__main__":
    ...
    sectorPair = create_ticker_sectorPair(spark)
    tickers = elaborate(spark, sectorPair)
    print(
